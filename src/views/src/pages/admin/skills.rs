...
/// Paginated skill listing for the admin table — real LIMIT/OFFSET in SQL.
#[server]
pub async fn admin_list_skills_page(page: i64) -> Result<(Vec<SkillView>, i64), ServerFnError> {
...
#[server]
pub async fn admin_delete_skill(skill_id: i32) -> Result<bool, ServerFnError> {
    crate::pages::admin::require_admin().await?;
    skill_svc()
        .await?
        .delete(skill_id)
...
pub fn AdminSkillsPage() -> impl IntoView {
    ...
    let items_resource = LocalResource::new(move || {
        refetch.get();
        admin_list_skills_page(page.get())
    });
    ...
    let f_progress = RwSignal::new("0".to_string());
    let f_star = RwSignal::new("0".to_string());
    ...
    let reset_form = move || {
        ...
        f_progress.set("0".to_string());
        f_star.set("0".to_string());
        error.set(None);
        upload_error.set(None);
    };
    ...
    let open_edit = move |row: SkillView| {
        editing_id.set(Some(row.skill_id));
        f_title.set(row.title);
        f_description.set(row.description);
        f_url_docs.set(row.url_docs);
        f_image.set(row.image_src);
        f_progress.set(row.progress.to_string());
        f_star.set(row.star.to_string());
        error.set(None);
        show_form.set(true);
    };
    ...
    let on_submit = move |ev: leptos::ev::SubmitEvent| {
        ...
        let progress = f_progress.get().parse::<i32>().unwrap_or(0).clamp(0, 100);
        let star = f_star.get().parse::<i32>().unwrap_or(0).clamp(0, 5);
        let input = modules::skills::SkillCommand {
            title: f_title.get(),
        ...
        leptos::task::spawn_local(async move {
            let result = match id {
                Some(id) => admin_update_skill(id, input).await.map(|_| ()),
                None => admin_create_skill(input).await.map(|_| ()),
            };
            match result {
                Ok(()) => {
                    saving.set(false);
    ...
    let on_delete = move |id: i32| {
        #[cfg(target_arch = "wasm32")]
        {
            if !leptos::web_sys::window()
                .and_then(|w| w.confirm_with_message("Hapus skill ini?").ok())
                .unwrap_or(false)
            {
                return;
            }
        ...
        leptos::task::spawn_local(async move {
            if admin_delete_skill(id).await.is_ok() {
                refetch.update(|n| *n += 1);
            }
        });
    };
        ...
        <AdminLayout>
            <div class="p-8 max-w-6xl">
                <div class="mb-8">
                    ...
                    <h2 class="text-3xl font-extrabold text-fg mb-1">"Skills"</h2>
                    <p class="text-muted text-sm">"Kelola tech stack yang ditampilkan di halaman Skills."</p>
                </div>

                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold text-fg">"Daftar Skill"</h3>
                    <button
                    ...
                    <form on:submit=on_submit class="bg-surface border border-line rounded-2xl p-5 mb-5">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                            ...
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-medium mb-1.5 text-fg">"Description"</label>
                            ...
                            <div>
                                <label class="block text-xs font-medium mb-1.5 text-fg">"Progress (0-100)"</label>
                                <input type="number" min="0" max="100" prop:value=f_progress on:input=move |e| f_progress.set(event_target_value(&e))
                            ...
                            <div>
                                <label class="block text-xs font-medium mb-1.5 text-fg">"Star (0-5)"</label>
                                <input type="number" min="0" max="5" prop:value=f_star on:input=move |e| f_star.set(event_target_value(&e))
                                    class="w-full px-3 py-2 bg-base border border-line rounded-lg text-fg focus:outline-none focus:border-teal-500 transition-colors text-sm"/>
                            </div>
                            <div class="sm:col-span-2">
                                <label class="block text-xs font-medium mb-1.5 text-fg">"Icon / Logo"</label>
                            ...
                            <table class="w-full text-sm text-fg">
                                <thead class="sticky top-0 bg-surface z-10">
                                    <tr class="border-b border-line">
                                        ...
                                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-muted uppercase tracking-wider w-32">"Progress"</th>
                                        <th class="text-left px-5 py-3.5 text-xs font-semibold text-muted uppercase tracking-wider w-24">"Star"</th>
                                ...
                                <tbody>
                                    {move || match items_resource.get() {
                                        Some(Ok((rows, _))) if rows.is_empty() => view! {
                                            <tr><td colspan="5" class="px-5 py-6 text-muted text-center text-xs">"Belum ada skill."</td></tr>
                                        }.into_any(),
                                        Some(Ok((rows, _))) => rows.into_iter().map(|row| {
                                            ...
                                            view! {
                                                <tr class="border-b border-line last:border-0 hover:bg-teal-500/5 transition-colors">
                                                    <td class="px-5 py-3.5 text-muted font-mono text-xs">{row.skill_id}</td>
                                                    <td class="px-5 py-3.5 text-fg font-medium">{row.title.clone()}</td>
                                                    <td class="px-5 py-3.5 text-muted text-xs">{format!("{}%", row.progress)}</td>
                                                    <td class="px-5 py-3.5 text-amber-400 text-xs">{"★".repeat(row.star.max(0) as usize)}</td>
