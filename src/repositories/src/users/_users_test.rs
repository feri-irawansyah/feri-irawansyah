...
async fn create_duplicate_email_fails(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    repo.create_async("dup@test.com", "pw", "First", 0)
        .await
        .unwrap();
    assert!(
        repo.create_async("dup@test.com", "pw", "Second", 0)
            .await
            .is_err()
    );
}
...
async fn find_by_id_finds_created_user(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    let created = repo
        .create_async("findid@test.com", "pw", "Find Me", 1)
        .await
        .unwrap();
...
async fn find_by_email_returns_none_for_unknown(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    assert!(
        repo.find_by_email_async("ghost@test.com")
            .await
            .unwrap()
            .is_none()
    );
...
async fn find_by_email_finds_created_user(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    repo.create_async("findemail@test.com", "pw", "Find Email", 0)
        .await
        .unwrap();
    let found = repo
        .find_by_email_async("findemail@test.com")
        .await
        .unwrap()
        .unwrap();
...
async fn find_all_respects_limit(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    for i in 0..5 {
        repo.create_async(&format!("u{i}@test.com"), "pw", "User", 0)
            .await
            .unwrap();
    }
...
async fn find_all_respects_offset(pool: PgPool) {
    let repo = UserRepositoryImpl::new(pool);
    repo.create_async("first@test.com", "pw", "First", 0)
        .await
        .unwrap();
    repo.create_async("second@test.com", "pw", "Second", 0)
        .await
        .unwrap();
